...
$stmt = $conn->prepare("SELECT lect_name, employee_no, email, phone_number, subject_teached, image_data FROM lecturers WHERE username = ?");
...
$stmt = $conn->prepare("SELECT employee_no FROM lecturers WHERE username = ?");
$stmt->bind_param("s", $username);
$stmt->execute();
$stmt->store_result();
