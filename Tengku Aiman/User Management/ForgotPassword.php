    ...
    if (empty($email) || empty($newPassword) || empty($confirmPassword)) {
        ...
        if ($stmt = $conn->prepare($query)) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                $updateQuery = "UPDATE student SET password = ? WHERE email = ?";
                $stmt->close();
            } else {
                // Check if email exists in the `lecturer` table
                $stmt->close();
                $query = "SELECT email FROM lecturer WHERE email = ?";
                if ($stmt = $conn->prepare($query)) {
                    $stmt->bind_param('s', $email);
                    $stmt->execute();
                    $stmt->store_result();
                    ...
                    if ($stmt->num_rows > 0) {
                        ...
                        $updateQuery = "UPDATE lecturer SET password = ? WHERE email = ?";
                    } else {
        ...
        <div class="login-wrapper">
            <form action="ForgotPassword.php" method="POST">
                ...
                <div class="input-field">
                    ...
                    <label style="color: white;">Please enter your UTM email</label>
                </div>
                ...
                <button type="button" id="login" style="margin-top: 10px;"><a href="login.php" style="color: black">Go to Login Page</a></button>
            </form>
        </div>
    </main>
